...
public class ServiceTache  {
    ...
    public ReponseDetailTache detail(Long id, Long idUtilisateur) {
        // 1. Récupérer l'utilisateur et la tâche depuis la base de données
        MUtilisateur user = repoUser.findById(idUtilisateur).get();
        MTache element = repo.findById(id).get();

        // 2. CORRECTION : Validation stricte du contrôle d'accès
        // Si la tâche demandée n'est pas dans la liste des tâches de l'utilisateur, on bloque l'accès.
        if (!user.taches.contains(element)) {
            throw new SecurityException("Accès refusé : Cette tâche ne vous appartient pas.");
        }

        // 3. Construction de la réponse si l'accès est autorisé
    ...
    public void deleteTache(Long id, Long idUtilisateur) {
        MUtilisateur user = repoUser.findById(idUtilisateur).get();
        MTache element = repo.findById(id).get();

        // CORRECTION : Validation du contrôle d'accès
        if (!user.taches.contains(element)) {
            throw new SecurityException("Accès refusé : Cette tâche ne vous appartient pas.");
        }

        user.taches.remove(element);
        // Note : pas besoin de repo.save(element) avant le delete
        repo.delete(element);
    }
