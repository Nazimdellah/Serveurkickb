...
class TestsServiceTache {
   ...
   void deleteTache(){
        ...
        assertEquals(1, serviceTache.accueil(alice.id).size());
        Long id = serviceTache.accueil(alice.id).getFirst().id;
        serviceTache.deleteTache(id,alice.id);
        try { RequeteInscription req2 = new RequeteInscription();
            req2.nom = "bob";
            req2.motDePasse = "Passw0rd!";
            req2.confirmationMotDePasse = "Passw0rd!";
            serviceAccount.inscrire(req2);
            MUtilisateur Bob = serviceTache.utilisateurParSonNom("bob");
            serviceTache.deleteTache(id,Bob.id);
        } catch (Exception e) {
            System.out.println("Bob Peut pas supprimé la tache d alice");
        }
